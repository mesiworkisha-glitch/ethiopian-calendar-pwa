...
(function (global) {
    ...
    function toJdn(date) {
        if (!date || !Number.isInteger(date.ey) || !Number.isInteger(date.em) || !Number.isInteger(date.ed)) throw new Error('Invalid Ethiopian date.');
    ...
    // These are the season families represented by the existing PWA.
    // Climatic and fasting seasons use the same boundaries/rules as app.js.
    const SEASON_CATALOG = {
        ...
        ].map((label, i) => ({ id: `lit-${i}`, labelKey: label, label }))
    };

    function seasonInfoForDate(date) {
        const bh = global.calculateBahreHasab(date.ey);
        const seasons = global.getSeasons(date.ey, date.em, date.ed, bh);
        const greatLentWeek = typeof global.getGreatLentWeek === 'function'
            ? global.getGreatLentWeek(global.ethiopianDayOfYear(date.em, date.ed), bh)
            : null;
        return {
            climatic: seasons.climatic,
            fasting: seasons.fasting,
            fastingProgress: seasons.progress || '',
            liturgical: seasons.liturgical,
            greatLentWeek: greatLentWeek ? global.t(greatLentWeek) : ''
        };
    }

    function matchesSeason(info, category, seasonIdOrLabel) {
        if (!category || !seasonIdOrLabel || seasonIdOrLabel === 'all') return true;
        if (category === 'climatic') {
            const item = SEASON_CATALOG.climatic.find(x => x.id === seasonIdOrLabel);
            return !!item && info.climatic === global.t(item.labelKey);
        }
        if (category === 'fasting') {
            const item = SEASON_CATALOG.fasting.find(x => x.id === seasonIdOrLabel);
            return !!item && info.fasting.includes(global.t(item.labelKey));
        }
        if (category === 'liturgical') return info.liturgical === seasonIdOrLabel;
        if (category === 'lent-week') return info.greatLentWeek === seasonIdOrLabel;
        return false;
    }

    function getSeasonCatalog() {
        return {
            climatic: SEASON_CATALOG.climatic.map(x => ({ ...x, label: global.t(x.labelKey) })),
            fasting: SEASON_CATALOG.fasting.map(x => ({ ...x, label: global.t(x.labelKey) })),
            liturgical: SEASON_CATALOG.liturgical.map(x => ({ ...x })),
            'lent-week': [
                'lent_week_1','lent_week_2','lent_week_3','lent_week_4','lent_week_5','lent_week_6','lent_week_7','lent_week_8'
            ].map(k => ({ id: k, labelKey: k, label: global.t(k) }))
        };
    }
    ...
    function generateSchedule(options) {
        ...
        const seasonCategory = options.seasonCategory || 'all';
        const seasonId = options.seasonId || 'all';
        const rows = [];
        let current = start;
        let guard = 0;

        while (toJdn(current) < endJdn && guard++ < 100000) {
            const season = seasonInfoForDate(current);
            if (matchesSeason(season, seasonCategory, seasonId)) {
                rows.push({ id: `${current.ey}-${current.em}-${current.ed}`, date: { ...current }, season, title: '', details: '', status: 'planned' });
            }
    ...
    function savePlan(plan) { const plans = loadPlans(); const index = plans.findIndex(p => p.id === plan.id); if (index >= 0) plans[index] = plan; else plans.push(plan); localStorage.setItem(STORAGE_KEY, JSON.stringify(plans)); return plan; }
    ...
    function exportText(plan, format) {
        ...
        const seasonHeader = plan.seasonCategory && plan.seasonCategory !== 'all' ? `Season Filter: ${plan.seasonId}` : 'Season Filter: All';
    ...
    function downloadPlan(plan, format) {
        ...
        const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = `${(plan.name || 'ethiopian-plan').replace(/[^\p{L}\p{N}_-]+/gu, '_')}.${extensions[format] || 'txt'}`; document.body.appendChild(a); a.click(); a.remove(); setTimeout(() => URL.revokeObjectURL(a.href), 0);
    }
