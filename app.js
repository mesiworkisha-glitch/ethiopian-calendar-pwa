const i18n = {
    ...
    om: {
        ...
        no_data: "Hanga ammaatti odeeffannoon hin galmoofne.", isl_year_events: "Ayyaanota Islaamaa Bara Kana",
    ...
    ti: {
        ...
        no_data: "ክሳብ ሕጂ ዝተመዝገበ መረጃ የለን።", isl_year_events: "በዓላት ምስልምና ናይዚ ዓመት",
...
function fNum(num) {
    return useGeezNumerals ? convertToGeez(parseInt(num)) : num;
}
...
function getAddisSunTimes(gDate) {
    ...
    return {
        rise: `${fNum(riseHr)}:${String(riseMin).padStart(2,'0')} ${t('txt_morning')}`,
        set: `${fNum(setHr - 12)}:${String(setMin).padStart(2,'0')} ${t('txt_evening')}`
    };
}
...
function hebrewMonthLength(year, month) {
    ...
    if (month === 2) return isComplete ? 30 : 29;
    if (month === 3) return isDeficient ? 29 : 30;
    if (month === 4) return 29;
    if (month === 5) return 30;
    if (month === 6) return isLeap ? 30 : 29;
    
    let mAfter = isLeap ? month - 7 : month - 6; 
    if (isLeap && month === 7) return 29;
    if (mAfter === 1) return 30;
    if (mAfter === 2) return 29;
    if (mAfter === 3) return 30;
    if (mAfter === 4) return 29;
    if (mAfter === 5) return 30;
    if (mAfter === 6) return 29;
    return 0;
}
...
function getHebrewMonthName(isLeap, m) {
    if (isLeap) return ["", "Tishrei", "Cheshvan", "Kislev", "Tevet", "Shevat", "Adar I", "Adar II", "Nisan", "Iyar", "Sivan", "Tammuz", "Av", "Elul"][m];
    return ["", "Tishrei", "Cheshvan", "Kislev", "Tevet", "Shevat", "Adar", "Nisan", "Iyar", "Sivan", "Tammuz", "Av", "Elul"][m];
}
...
function getSeasons(ey, em, ed, bh) {
    ...
    elseHere is the complete, improved `app.js` file. The `renderIslamic()` function and the translation dictionary (`i18n`) have been extensively updated so the Hijri tab precisely mirrors the detailed overview, year layout, month breakdown, and key date listings found in the Python script.
...
const i18n = {
    am: {
        ...
        err_cycle_range: "የዑደት ርዝመት ከ15 እስከ 60 ቀናት መካከል መሆን አለበት።", err_period_range: "የወር አበባ ቆይታ ከ1 እስከ 15 ቀናት መካከል መሆን አለበት።",
        err_future_date: "የገባው ቀን ወደፊት ስለሆነ እባክዎ ያለፈ ቀን ያስገቡ።", err_generic: "የተከሰተ ችግር አለ፤ እባክዎ የገቡት መረጃ ትክክል መሆኑን ያረጋግጡ።",
        no_data: "ምንም መረጃ አልተመዘገበም።", isl_year_events: "ሙሉ ዓመት መረጃ",
        lbl_day_num: "ቀን", lbl_weekday: "ዕለት", lbl_fasting_season: "የአጽዋም ዘመን", lbl_liturgical_season: "የቤተክርስቲያን ዘመን",
        lbl_events_memorials: "በዓላት / መታሰቢያ", lbl_month_events: "በዚህ ወር ያሉ በዓላትና አጽዋማት", lbl_year_movable_events: "የዓመቱ ተንቀሳቃሽ በዓላትና አጽዋማት",
        txt_sunrise: "መውጫ", txt_sunset: "መግቢያ", lbl_upcoming: "ቀጣይ በዓላትና አጽዋማት", lbl_today_synax: "የዕለቱ ስንክሳር በዓላት",
        lbl_annual_feasts: "ዓመታዊ በዓላት", lbl_monthly_feasts: "ወርኃዊ በዓላት", lbl_national_short: "ብሔራዊ በዓላት", lbl_movable_short: "ተንቀሳቃሽ በዓላት", lbl_synax_short: "የስንክሳር በዓላት",
        ...
        hol_ashura: "ዓሹራ (Ashura)", hol_mawlid: "መውሊድ (Mawlid)", hol_isra: "እስራ ወሚዕራጅ (Isra and Mi'raj)", hol_ramadan: "ረመዳን (Ramadan)", hol_laylat: "Laylat al‑Qadr", hol_eid_fitr: "ዒድ አልፊጥር (Eid al-Fitr)", hol_arafah: "የዐረፋ ቀን (Day of Arafah)", hol_eid_adha: "ዒድ አልአድሐ (Eid al-Adha)", hol_hajj: "Hajj",
        bh_medeb: "መደብ", bh_wenber: "ወንበር", bh_tinte: "ጥንተ ቀመር", bh_metqe: "መጥቅዕ", bh_abekte: "አበቅቴ", bh_hamer: "መባጃ ሐመር",
        isl_leap_year: "ዓመቱ ሰበቅ (ልዩ) ዓመት ነው።", isl_reg_year: "ዓመቱ መደበኛ ዓመት ነው።", isl_no_events: "የተለየ የጾም ወይም የሐጅ ወቅት አይደለም።", isl_disclaimer: "ማሳሰቢያ፦ ይህ በሒሳብ ስሌት (tabular Islamic calendar) ላይ የተመሠረተ ሂሳባዊ ቀን ሲሆን ከትክክለኛ የጨረቃ ምልከታ ጋር በ1-2 ቀናት ሊለያይ ይችላል።", isl_ramadan_start: "የረመዳን መጀመሪያ", isl_ramadan_last_10: "የረመዳን መጨረሻዎቹ 10 ቀናት ይጀምራሉ", lbl_equiv_greg: "ተመጣጣኝ የግሪጎሪያን ቀን", lbl_equiv_eth: "ተመጣጣኝ የኢትዮጵያ ቀን", lbl_current_info: "ወቅታዊ መረጃ", lbl_months: "ወራት", lbl_key_dates: "ዋና ዋና ዕለታት"
    },
    en: {
        app_title: "Ethiopian Calendar & Bahire Hasab",
        intro_desc: "Free, offline-capable app: Accurate Bahire Hasab, movable feasts, fasting seasons, Synaxarium search, and conversion for Gregorian, Julian, Hebrew, and Hijri dates.",
        ...
        err_cycle_range: "Cycle length must be between 15 and 60 days.", err_period_range: "Period length must be between 1 and 15 days.",
        err_future_date: "That date is in the future; please enter a past date.", err_generic: "Something went wrong; please check your input.",
        no_data: "No data recorded yet.", isl_year_events: "Full Year Overview",
        lbl_day_num: "Day", lbl_weekday: "Weekday", lbl_fasting_season: "Fasting Season", lbl_liturgical_season: "Church Season",
        lbl_events_memorials: "Holidays / Memorials", lbl_month_events: "Holidays & Fasts This Month", lbl_year_movable_events: "Movable Feasts & Fasts This Year",
        txt_sunrise: "Rise", txt_sunset: "Set", lbl_upcoming: "Upcoming Feasts & Fasts", lbl_today_synax: "Today's Synaxarium Feasts",
        lbl_annual_feasts: "Annual Feasts", lbl_monthly_feasts: "Monthly Feasts", lbl_national_short: "National Holidays", lbl_movable_short: "Movable Feasts", lbl_synax_short: "Synaxarium Feasts",
        ...
        hol_ashura: "Ashura", hol_mawlid: "Mawlid", hol_isra: "Isra and Mi'raj", hol_ramadan: "Ramadan", hol_laylat: "Laylat al-Qadr", hol_eid_fitr: "Eid al-Fitr", hol_arafah: "Day of Arafah", hol_eid_adha: "Eid al-Adha", hol_hajj: "Hajj",
        bh_medeb: "Medeb", bh_wenber: "Wenber", bh_tinte: "Tinte Qemer", bh_metqe: "Metqe", bh_abekte: "Abekte", bh_hamer: "Mebaja Hamer",
        isl_leap_year: "This is a Leap Year.", isl_reg_year: "This is a Regular Year.", isl_no_events: "No specific fasting or Hajj season.", isl_disclaimer: "Note: This is based on the tabular Islamic calendar and may differ from actual moon sightings by 1-2 days.", isl_ramadan_start: "Start of Ramadan", isl_ramadan_last_10: "Last 10 days of Ramadan begin", lbl_equiv_greg: "Equivalent Gregorian Date", lbl_equiv_eth: "Equivalent Ethiopian Date", lbl_current_info: "Current Information", lbl_months: "Months", lbl_key_dates: "Key Dates"
    },
    om: {
        app_title: "Kalaandarii Itoophiyaa & Bahire Hasab",
        intro_desc: "Appii tolaa, sarara malee hojjetu: Herrega Bahire Hasab sirrii, ayyaanoota, yeroo soomaa, fi jijjiirraa guyyaa (Gregorian, Julian, Hebrew, Hijri).",
        ...
        hol_ashura: "Aashuraa", hol_mawlid: "Mawliid", hol_isra: "Isra fi Mi'raaj", hol_ramadan: "Ramadaana", hol_laylat: "Laylat al-Qadr", hol_eid_fitr: "Iid Al-Faxir", hol_arafah: "Arafa", hol_eid_adha: "Iid Al-Adhaa", hol_hajj: "Hajjii",
        bh_medeb: "Medeb", bh_wenber: "Wenber", bh_tinte: "Tinte Qemer", bh_metqe: "Metqe", bh_abekte: "Abekte", bh_hamer: "Mebaja Hamer",
        isl_leap_year: "Bara Utaalchaati.", isl_reg_year: "Bara Idileeti.", isl_no_events: "Yeroo addaa soomaa ykn hajjii miti.", isl_disclaimer: "Hubachiisa: Kun herrega (tabular Hijri) irratti kan hundaa'eedha, akkasumas ji'a arguu irraa guyyaa 1-2 garaagarummaa qabaachuu danda'a.", isl_ramadan_start: "Jalqaba Ramadaanaa", isl_ramadan_last_10: "Guyyoota 10 dhumaa Ramadaanaa", lbl_equiv_greg: "Guyyaa Gregorian Walgitu", lbl_equiv_eth: "Guyyaa Itoophiyaa Walgitu", lbl_current_info: "Odeeffannoo Yeroo", lbl_months: "Ji'oota", lbl_key_dates: "Guyyoota Ijoo"
    },
    ti: {
        app_title: "ካላንደር ኢትዮጵያን ባሕረ ሓሳብን",
        intro_desc: "ብነጻን ብዘይ ኢንተርነትን ዝሰርሕ ኣፕሊኬሽን፦ ቅኑዕ ስሌት ባሕረ ሓሳብ፣ ተንቀሳቐስቲ በዓላት፣ እዋናት ጾም፣ መርበብ ስንክሳርን መቐየሪ ዕለታትን።",
        ...
        hol_ashura: "ዓሹራ", hol_mawlid: "መውሊድ", hol_isra: "እስራ ወሚዕራጅ", hol_ramadan: "ረመዳን", hol_laylat: "ለይለተል ቀድር", hol_eid_fitr: "ዒድ ኣልፈጥር", hol_arafah: "መዓልቲ ዓረፋ", hol_eid_adha: "ዒድ ኣልኣድሓ", hol_hajj: "ሓጅ",
        bh_medeb: "መደብ", bh_wenber: "ወንበር", bh_tinte: "ጥንተ ቀመር", bh_metqe: "መጥቅዕ", bh_abekte: "ኣበቅቴ", bh_hamer: "መባጃ ሓመር",
        isl_leap_year: "ዓመቱ ዘመነ ሰበቅ (ፍሉይ) እዩ።", isl_reg_year: "ዓመቱ ስሩዕ ዓመት እዩ።", isl_no_events: "ፍሉይ እዋን ጾም ወይ ሓጅ ኣይኮነን።", isl_disclaimer: "መተሓሳሰቢ፦ እዚ ኣብ ስሌት (tabular Hijri) ዝተመርኮሰ ኮይኑ፡ ምስ ርእየት ወርሒ ብ1-2 መዓልታት ክፈላለ ይኽእል እዩ።", isl_ramadan_start: "መጀመርታ ረመዳን", isl_ramadan_last_10: "ናይ መወዳእታ 10 መዓልታት ረመዳን ይጅምሩ", lbl_equiv_greg: "መዘና ዕለት ግሪጎሪያን", lbl_equiv_eth: "መዘና ዕለት ኢትዮጵያ", lbl_current_info: "እዋናዊ ሓበሬታ", lbl_months: "ኣዋርሕ", lbl_key_dates: "ዓበይቲ ዕለታት"
    }
};
...
function getIslamicEvents(im, id) {
    ...
    if (im === 1 && id === 10) ev.push(t('hol_ashura')); if (im === 3 && id === 12) ev.push(t('hol_mawlid'));
    if (im === 7 && id === 27) ev.push(t('hol_isra'));
    if (im === 9) { ev.push(t('hol_ramadan')); if (id >= 21) ev.push(t('isl_ramadan_last_10') || t('hol_laylat')); }
    if (im === 10 && id === 1) ev.push(t('hol_eid_fitr'));
    if (im === 12) { if (id === 9) ev.push(t('hol_arafah')); if (id === 10) ev.push(t('hol_eid_adha')); if (id <= 10) ev.push(t('hol_hajj')); }
    return ev;
}
...
function getSeasons(ey, em, ed, bh) {
    ...
    for (let r of ranges) {
        if (dayNum >= r.s && dayNum < r.s + r.l) {
            progress = `(${r.n}: ${fNum(dayNum - r.s + 1)} / ${fNum(r.l)})`;
            break;
        }
    }
...
function ethiopianDayOfYear(em, ed) {
    return (em - 1) * 30 + ed;
}

function dayOfYearToMonthDay(dayNum) {
    let em = Math.floor((dayNum - 1) / 30) + 1;
    let ed = ((dayNum - 1) % 30) + 1;
    return { em, ed };
}
...
function getNamedEventsForYear(ey) {
    ...
    let fixed = [
        ...
        [131, "hol_timkat"],
        [331, "fast_filseta"],
        [345, "hol_filseta_maryam"]
    ];
    fixed.forEach(e => events.push(e));
    let gennaDay = (ey % 4 === 0) ? 28 : 29;
    events.push([90 + gennaDay, "hol_genna"]);

    for (const [internalKey, dateObj] of Object.entries(bh.feasts)) {
        let i18nKey = ["nenewe", "abiy", "hawaryat", "dihnet"].includes(internalKey) ? `fast_${internalKey}` : `fest_${internalKey}`;
        let dayNum = (dateObj.m - 1) * 30 + dateObj.d;
        events.push([dayNum, i18nKey]);
    }
...
function getFdreHolidays(ey) {
    let startG = ethToGregorian(ey, 1, 1);
    let yearG = startG.getFullYear();
    let bh = calculateBahreHasab(ey);
...
function getNationalHolidaysMap(ey) {
    ...
    holidays.forEach(item => {
        let g = item.g;
        let eth = gregorianToEthiopian(g.getFullYear(), g.getMonth()+1, g.getDate());
        let dayNum = ethiopianDayOfYear(eth.em, eth.ed);
        if (!map[dayNum]) map[dayNum] = [];
        map[dayNum].push(item.n);
    });
    return map;
}
...
function getYearlyEvents(ey) {
    let events = [];
    let named = getNamedEventsForYear(ey);
    named.forEach(([dayNum, key]) => {
        events.push({ dayNum, label: t(key), type: 'movable' });
    });

    let natMap = getNationalHolidaysMap(ey);
    for (let dayNum in natMap) {
        natMap[dayNum].forEach(label => {
            events.push({ dayNum: parseInt(dayNum), label, type: 'national' });
        });
    }

    events.sort((a,b) => a.dayNum - b.dayNum);
    return events;
}
...
function getMonthDays(ey, em) {
    ...
    for (let ed = 1; ed <= monthLen; ed++) {
        days.push(getDayDetails(ey, em, ed));
    }
    return days;
}
...
async function renderYearSearch(ey, out) {
    ...
    html += `<h4>${t('lbl_bahire')}</h4><ul>
        ...
        <li>${t('lbl_wengelawi')}: ዘመነ ${bh.wengelawi} (ዓመተ ዓለም ${fNum(bh.aa)})</li>
    </ul>`;
    ...
    html += `<h4>${t('lbl_year_movable_events')}</h4><ul>`;
    let shown = new Set();
    events.forEach(ev => {
        let key = ev.label;
        if (!shown.has(key)) {
...
async function renderMonthSearch(ey, em, out) {
    ...
    let html = `<h3>${mList[em]} ${fNum(ey)} ${t('txt_year')}</h3>`;
    html += `<p>${formatDate(startG)} - ${formatDate(endG)} (${fNum(monthLen)} ${t('txt_days')})</p>`;
    ...
    html += `<tr><th>${t('lbl_day_num')}</th><th>${t('lbl_weekday')}</th><th>${t('lbl_fasting_season')}</th><th>${t('lbl_liturgical_season')}</th><th>${t('lbl_events_memorials')}</th></tr>`;
    for (let ed = 1; ed <= monthLen; ed++) {
        let dayNum = ethiopianDayOfYear(em, ed);
        let gDate = ethToGregorian(ey, em, ed);
        let weekday = wList[gDate.getDay()];
        ...
        yearlyEvents.forEach(ev => {
            if (ev.dayNum === dayNum) dayEvents.push(ev.label);
        });
    ...
    let monthEvents = yearlyEvents.filter(ev => {
        let { em: evEm } = dayOfYearToMonthDay(ev.dayNum);
        return evEm === em;
    });
    let monthHolidays = [];
    for (let dayNum in holidaysMap) {
        let { em: hEm } = dayOfYearToMonthDay(parseInt(dayNum));
        if (hEm === em) {
            holidaysMap[dayNum].forEach(label => monthHolidays.push({ dayNum: parseInt(dayNum), label }));
        }
    }
...
async function renderIslamicYearSearch(iy, out) {
    ...
    let wList = getWeekdays();
    let html = `<h3>${fNum(iy)} ${t('lbl_hijri')}</h3><h4>${t('isl_year_events')}</h4><ul>`;
    let any = false;
    for (let im = 1; im <= 12; im++) {
        ...
        for (let id = 1; id <= monthLen; id++) {
            ...
            if (evs.length) {
                any = true;
                let jdn = islamicToJdn(iy, im, id);
                let gDate = jdnToGregorian(jdn);
                html += `<li>${evs.join('፣ ')} — ${islMonths[im]} ${fNum(id)}፣ ${wList[gDate.getDay()]} (${formatDate(gDate)})</li>`;
            }
        }
    }
    if (!any) html += `<li>${t('no_data')}</li>`;
    html += `</ul>`;
    out.innerHTML = html;
}
...
async function renderIslamicMonthSearch(iy, im, out) {
    ...
    let islMonths = t('islamic_months');
    let wList = getWeekdays();
    let startG = jdnToGregorian(islamicToJdn(iy, im, 1));
    let endG = jdnToGregorian(islamicToJdn(iy, im, monthLen));

    let html = `<h3>${islMonths[im]} ${fNum(iy)} ${t('lbl_hijri')}</h3>`;
    html += `<p>${formatDate(startG)} - ${formatDate(endG)} (${fNum(monthLen)} ${t('txt_days')})</p>`;
    html += `<table border="1" style="border-collapse:collapse; width:100%;">`;
    html += `<tr><th>${t('lbl_day_num')}</th><th>${t('lbl_weekday')}</th><th>${t('lbl_gregorian')}</th><th>${t('lbl_ethiopian')}</th><th>${t('holidays_title')}</th></tr>`;
    for (let id = 1; id <= monthLen; id++) {
        let jdn = islamicToJdn(iy, im, id);
        let gDate = jdnToGregorian(jdn);
        ...
        let mList = getMonths();
        let evs = getIslamicEvents(im, id);
        html += `<tr><td>${fNum(id)}</td><td>${wList[gDate.getDay()]}</td><td>${formatDate(gDate)}</td><td>${mList[eDate.em]} ${fNum(eDate.ed)}</td><td>${evs.join('፣ ')}</td></tr>`;
    }
    html += `</table>`;
    out.innerHTML = html;
}
...
async function renderFullDateSearch(ey, em, ed, out) {
    ...
    let holidaysMap = getNationalHolidaysMap(ey);
    let dayNum = ethiopianDayOfYear(em, ed);
    let nationalHolidays = holidaysMap[dayNum] || [];

    let yearlyEvents = getYearlyEvents(ey);
    let dayEvents = yearlyEvents.filter(ev => ev.dayNum === dayNum).map(ev => ev.label);
    ...
    if (nationalHolidays.length) {
        html += `<p><strong>${t('lbl_national_short')}:</strong> ${nationalHolidays.join('; ')}</p>`;
    }
    if (dayEvents.length) {
        html += `<p><strong>${t('lbl_movable_short')}:</strong> ${dayEvents.join('; ')}</p>`;
    }
    if (synaxEntries.length) {
        html += `<p><strong>${t('lbl_synax_short')}:</strong> ${synaxEntries.join('; ')}</p>`;
    }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    modules.forEach(m => {
        try { m.func(); } catch (err) { console.error(`Error loading module [${m.name}]:`, err); }
    });
});
...
function refreshLiveOutputs() {
    ...
    const periodicOut = document.getElementById('periodic-output');
    if (periodicOut && periodicOut.innerHTML !== "") {
        const form = document.getElementById('periodic-form');
        if (form) form.dispatchEvent(new Event('submit'));
    }

    const converterOut = document.getElementById('converter-output');
    if (converterOut && converterOut.innerHTML !== "") {
        const btnConvert = document.getElementById('btn-convert');
        if (btnConvert) btnConvert.click();
    }

    const ageOut = document.getElementById('age-output');
...
async function renderToday() {
    ...
    <ul>
        ...
        <li><strong>${t('lbl_season')}:</strong> ${seasons.climatic} | ${seasons.liturgical}</li>
        <li><strong>${t('lbl_fasting')}:</strong> ${seasons.fasting} ${seasons.progress ? "<br><em>" + seasons.progress + "</em>" : ""}</li>
        <li><strong>${t('lbl_moon')}:</strong> ${moonPhase} (${t('txt_day')} ${fNum(cherekaAge)})</li>
        ...
        <li><strong>${t('lbl_awde')}:</strong> ${getAwdeNegestSign(now.getMonth()+1, now.getDate())}</li>
    </ul>`;

    html += `<h3>${t('lbl_upcoming')}</h3>`;
    ...
    if (todayHolidays.length) {
        html += `<p style="background:rgba(240,180,41,0.15); padding:10px; border-left:4px solid #f0b429;"><strong>${t('holidays_title')}፦</strong> ${todayHolidays.join('፣ ')}</p>`;
    }
    ...
    if (entries.length > 0) {
        ...
        if (annualFeasts.length > 0) {
            html += `<h4>${t('lbl_annual_feasts')}</h4><ul>` + annualFeasts.map(e => `<li>${e}</li>`).join('') + `</ul>`;
        }
        if (monthlyFeasts.length > 0) {
            html += `<h4>${t('lbl_monthly_feasts')}</h4><ul>` + monthlyFeasts.map(e => `<li>${e}</li>`).join('') + `</ul>`;
        }
    } else {
        html += `<p>${t('no_synax')}</p>`;
    }
...
function renderIslamic() {
    ...
    let ev = getIslamicEvents(isl.im, isl.id);
    let wList = getWeekdays();
    let islMonths = t('islamic_months');
    let mList = getMonths();

    let eth = jdnToEthiopian(jdn);
    let isLeap = isIslamicLeap(isl.iy);
    let leapText = isLeap ? t('isl_leap_year') : t('isl_reg_year');

    // Build Current Date Overview mirroring Python HTML format
    let html = `<h3>${wList[now.getDay()]}፣ ${islMonths[isl.im]} ${fNum(isl.id)} ${t('txt_day')} ${fNum(isl.iy)} ${t('lbl_hijri')}</h3>`;
    html += `<p>${t('lbl_equiv_greg')}፦ ${formatDate(now)}</p>`;
    html += `<p>${t('lbl_equiv_eth')}፦ ${mList[eth.em]} ${fNum(eth.ed)} ${t('txt_day')} ${fNum(eth.ey)} ${t('txt_year')}</p>`;

    html += `<h4>${t('lbl_current_info')}</h4><ul>`;
    if (ev.length > 0) {
        ev.forEach(e => { html += `<li>${e}</li>`; });
    } else {
        html += `<li>${t('isl_no_events')}</li>`;
    }
    html += `<li>${leapText}</li>`;
    html += `</ul>`;
    html += `<p><em>${t('isl_disclaimer')}</em></p>`;

    // Build Full Year Overview mirroring Python HTML format
    let yearStartJdn = islamicToJdn(isl.iy, 1, 1);
    let yearEndJdn = islamicToJdn(isl.iy, 12, getIslamicMonthLength(isl.iy, 12));
    let yearStartG = jdnToGregorian(yearStartJdn);
    let yearEndG = jdnToGregorian(yearEndJdn);
    let startEth = jdnToEthiopian(yearStartJdn);
    let endEth = jdnToEthiopian(yearEndJdn);
    ...
    for (let i = 1; i <= 12; i++) {
        let mLen = getIslamicMonthLength(isl.iy, i);
        let mStartJdn = islamicToJdn(isl.iy, i, 1);
        let mEndJdn = islamicToJdn(isl.iy, i, mLen);
        let mStartG = jdnToGregorian(mStartJdn);
        let mEndG = jdnToGregorian(mEndJdn);
        let mStartEth = jdnToEthiopian(mStartJdn);
        let mEndEth = jdnToEthiopian(mEndJdn);
    ...
    html += `<h4>${t('lbl_key_dates')}</h4><ul>`;
    let keyEvents = [
        [1, 10, t('hol_ashura')],
        [3, 12, t('hol_mawlid')],
        [7, 27, t('hol_isra')],
        [9, 1, t('isl_ramadan_start')],
        [9, 21, t('isl_ramadan_last_10')],
        [10, 1, t('hol_eid_fitr')],
        [12, 9, t('hol_arafah')],
        [12, 10, t('hol_eid_adha')]
    ];
    keyEvents.forEach(ke => {
        let evJdn = islamicToJdn(isl.iy, ke[0], ke[1]);
        let evG = jdnToGregorian(evJdn);
        let evEth = jdnToEthiopian(evJdn);
    ...
    btn.addEventListener('click', async () => {
        ...
        try {
            ...
            if (type === 'eth') {
                ...
                let jdn = (type === 'julian') ? julianToJdn(y, m, d) : hebrewToJdn(y, m, d);
                let eDate = jdnToEthiopian(jdn);
                await renderFullDateSearch(eDate.ey, eDate.em, eDate.ed, out);
            } else if (type === 'hijri') {
                m = mStr ? parseInt(mStr) : null;
                ...
                if (y === null) {
                    out.innerHTML = `<p style="color:red;">${t('err_enter_date')}</p>`; return;
                }
                if (d !== null && m !== null) {
                    if (d < 1 || d > getIslamicMonthLength(y, m)) {
                        out.innerHTML = `<p style="color:red;">${t('err_invalid_day')}</p>`; return;
                    }
                    let jdn = islamicToJdn(y, m, d);
                    let eDate = jdnToEthiopian(jdn);
                    await renderFullDateSearch(eDate.ey, eDate.em, eDate.ed, out);
                } else if (m !== null) {
                    await renderIslamicMonthSearch(y, m, out);
                } else {
                    await renderIslamicYearSearch(y, out);
                }
            }
        } catch (err) {
        ...
        for (let mName in data) {
            for (let dNum in data[mName]) {
                data[mName][dNum].forEach(entry => {
                    if (normalizeAmharic(entry).includes(q)) matches.push({ mName, dNum: parseInt(dNum), entry });
                });
            }
        }
...
function setupAgeCalc() {
    ...
    btn.addEventListener('click', () => {
        ...
        try {
            ...
            let years = cY - bY;
            let annivDay = Math.min(bD, getMonthLength(bY + years, bM));
            let annivJdn = ethiopianToJdn(bY + years, bM, annivDay);
            if (annivJdn > cJdn) {
                years--;
                annivDay = Math.min(bD, getMonthLength(bY + years, bM));
                annivJdn = ethiopianToJdn(bY + years, bM, annivDay);
            }
            ...
            for (let i = 0; i < 13; i++) {
                ...
                let nextDay = Math.min(bD, getMonthLength(nextY, nextM));
                let nextJdn = ethiopianToJdn(nextY, nextM, nextDay);
                if (nextJdn > cJdn) break;
                months++;
                curY = nextY; curM = nextM; curJdn = nextJdn;
            }

            let days = cJdn - curJdn;
...
function initFooter() {
    ...
    if (el) el.textContent = fNum(new Date().getFullYear());
}
...
function setupClipboardCopy() {
    ...
    if (copyEthBtn) {
        copyEthBtn.addEventListener('click', async () => {
            ...
            let mList = getMonths(), wList = getWeekdays();
            let msg = `${wList[now.getDay()]}፣ ${mList[eth.em]} ${fNum(eth.ed)} ${t('txt_day')} ${fNum(eth.ey)} ${t('txt_year')}`;
            try {
                await navigator.clipboard.writeText(msg);
                announceStatus(t('txt_copied'));
            } catch (e) {
                announceStatus(t('txt_fail'));
            }
        });
    }
}