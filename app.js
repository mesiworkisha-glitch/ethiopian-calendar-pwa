...
async function buildTodayFullDetails() {
    ...
    let data = await loadSynaxarium(), amharicMonths = ["", "መስከረም", "ጥቅምት", "ኅዳር", "ታኅሣሥ", "ጥር", "የካቲት", "መጋቢት", "ሚያዝያ", "ግንቦት", "ሰኔ", "ሐምሌ", "ነሐሴ", "ጳጉሜ"];
...
function setupSynaxarium() {
    ...
    if (!btn) return;
    btn.addEventListener('click', async () => {
        let q = normalizeAmharic(document.getElementById('synax-query').value.trim()), out = document.getElementById('synax-output'); if (!out || !q) return;
        let data = await loadSynaxarium(), matches = [];
